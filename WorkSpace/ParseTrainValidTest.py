...
for f in os.listdir(path='../CleaningWithSpeed'):
...
tr_out = gzip.open("../TrainValidTest/training_set_top_8.json.gz", 'wb')
va_out = gzip.open("../TrainValidTest/validation_set_top_8.json.gz", 'wb')
te_out = gzip.open("../TrainValidTest/test_set_top_8.json.gz", 'wb')

for workout in train_list:
    tr_out.write(workout)
print("Finished writing training set for top 8")

for workout in validation_list:
    va_out.write(workout)
print("Finished writing validation set for top 8")

for workout in test_list:
    te_out.write(workout)
print("Finished writing test set for top 8")
